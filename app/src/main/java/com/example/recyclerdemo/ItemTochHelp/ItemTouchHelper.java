...
package com.example.recyclerdemo.ItemTochHelp;
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
        ...
        public void onTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            switch (action) {
                case MotionEvent.ACTION_MOVE: {
                    ...
                    float abs=Math.abs(initX-event.getX());
    ...
    public void doClickEvent(MotionEvent event) {
    ...
    /**
     * @param consumeView 当前的itemView
     * @param x
     * @param y
     */
    private View findConsumeView(ViewGroup consumeView, float x, float y) {

        int childCount = consumeView.getChildCount();

        for (int i = 0; i < childCount; i++) {

            View childView = consumeView.getChildAt(i);

            if (childView instanceof ViewGroup && childView.getVisibility() == View.VISIBLE) {

                View view = findConsumeView((ViewGroup) childView, x, y);
                ...
                boolean clickArea = isClickArea(childView, (int) x, (int) y);
            ...
            if (isClickArea(consumeView, (int) x, (int) y)) {
            }
     ...
     * 是否在点击的范围内
     */
    private boolean isClickArea(View child, int x, int y) {

        int[] rec = new int[2];

        child.getLocationOnScreen(rec);

        Rect rect = new Rect(rec[0], rec[1], child.getWidth() + rec[0], child.getHeight() + rec[1]);


        if (rect.contains(x, y) && child.hasOnClickListeners() && child.getVisibility() == View.VISIBLE) {
            return true;
